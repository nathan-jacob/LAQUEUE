...
std::mutex mylock;
//MY NEW FUNCTION THAT IS NEWLY IMPLEMENTED

//MADE BY NATHAN JACOB
lqd_t lq_open (const char *__name, int __oflag, int character, char fakestring[], bool completed)
{
    for (int i=0; i < LQUEUE_TABLE_MAX; i++) {
        if(strcmp(lqueue_table[i].lq_name, __name) == 0)
            ...
            if (!completed) {
                fakestring = (char *) &__name;
                character = __oflag;
                strcmp(reinterpret_cast<const char *>(fakestring[i]), __name);
                completed = true;
            }
        }
    }
